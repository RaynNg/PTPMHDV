...
def fetch_metrics_data() -> dict:
    ...
    queries = {
        ...
        "checkout_latency_p95": "histogram_quantile(0.95, rate(demo_checkout_duration_ms_bucket[1m]))",
        "checkout_latency_p50": "histogram_quantile(0.50, rate(demo_checkout_duration_ms_bucket[1m]))",
    ...
    <html lang=\"en\">
      <head>
        ...
        <style>
          ...
          pre { background: #f7f7f7; border: 1px solid #ddd; padding: 12px; border-radius: 8px; }
        </style>
      </head>
      <body>
        <h2>Microservice Observability Demo</h2>
        <p>API Gateway đang gọi tới Orders + Inventory và gửi telemetry qua OpenTelemetry.</p>
        <button onclick=\"checkout()\">Gọi 1 request checkout</button>
        <button onclick=\"startLoad()\">Bắn tải 3 req/s</button>
        <button onclick=\"stopLoad()\">Dừng tải</button>
        <pre id=\"out\">Sẵn sàng...</pre>
        <script>
          let timer = null;
          async function checkout() {
            const r = await fetch('/api/checkout?item_id=item-1&qty=' + (Math.floor(Math.random()*3)+1));
            const data = await r.json();
            const formatted = JSON.stringify(data, null, 2);
            document.getElementById('out').textContent = formatted;

            // Open observability page in new tab if trace_id is available
            if (data.trace_id) {
          ...
          function startLoad() {
            if (timer) return;
            timer = setInterval(() => checkout(), 330);
            document.getElementById('out').textContent = 'Đang tạo traffic...';
          }
          function stopLoad() {
            clearInterval(timer);
            timer = null;
            document.getElementById('out').textContent = 'Đã dừng traffic';
          }
        </script>
      </body>
    </html>
      ...
      <body>
        ...
        <script>
          ...
          function updateMetrics(metrics) {{
            ...
            const totalReq = parseMetric(metrics.total_requests?.[0]?.value?.[1]);
            const errRate = parseMetric(metrics.error_rate?.[0]?.value?.[1]);
            const p95 = parseMetric(metrics.checkout_latency_p95?.[0]?.value?.[1]);
            const reqRate = parseMetric(metrics.checkout_rate?.[0]?.value?.[1]);

            document.getElementById('totalRequests').textContent = Math.round(totalReq);
            document.getElementById('errorRate').textContent = toFixedSafe(errRate * 100, 2) + '%';
            document.getElementById('p95Latency').textContent = Math.round(p95);
            ...
            latencyChart.data.datasets[0].data.push(Math.round(p95));
            const p50 = parseMetric(metrics.checkout_latency_p50?.[0]?.value?.[1]);
            latencyChart.data.datasets[1].data.push(Math.round(p50));
...
def checkout(item_id: str = Query(default="item-1"), qty: int = Query(default=1)):
    ...
    with tracer.start_as_current_span("checkout_flow") as span:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status_code, detail=f"Checkout failed: {detail}"
            )
            ...
            raise HTTPException(status_code=500, detail=f"Checkout failed: {str(exc)}")
